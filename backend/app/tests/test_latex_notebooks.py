...
async def _create_group(client, token: str, book_id: int, name: str = "Mechanics"):
    return await client.post(
        f"{API}/books/{book_id}/latex-notebook-groups",
        headers=_auth_headers(token),
        json={"name": name},
    )


async def _create_notebook(client, token: str, book_id: int, **overrides):
    payload = {"title": "Problem 1", "content": LATEX_SOURCE, "page": 12}
    payload.update(overrides)
    return await client.post(
        f"{API}/books/{book_id}/latex-notebooks",
        headers=_auth_headers(token),
        json=payload,
    )
...
@pytest.mark.asyncio
async def test_create_latex_group_and_notebook(client):
    _, token = await _token(client)
    book_id = await _make_book(client, token)
    group = (await _create_group(client, token, book_id)).json()

    response = await _create_notebook(client, token, book_id, group_id=group["id"])
    ...
    assert body["content"] == LATEX_SOURCE
    assert body["page"] == 12
...
async def test_list_groups_ordered_by_position(client):
    ...
    renamed = await client.patch(
        f"{API}/books/{book_id}/latex-notebook-groups/{second['id']}",
        headers=_auth_headers(token),
    ...
    response = await client.get(
        f"{API}/books/{book_id}/latex-notebook-groups", headers=_auth_headers(token)
    )
...
@pytest.mark.asyncio
async def test_list_notebooks_newest_first(client):
    _, token = await _token(client)
    book_id = await _make_book(client, token)
    first = (await _create_notebook(client, token, book_id, title="First")).json()
    second = (await _create_notebook(client, token, book_id, title="Second")).json()

    response = await client.get(
        f"{API}/books/{book_id}/latex-notebooks", headers=_auth_headers(token)
    )
    assert response.status_code == 200
    assert [n["id"] for n in response.json()] == [second["id"], first["id"]]


@pytest.mark.asyncio
async def test_patch_notebook_clears_group_and_page(client):
    _, token = await _token(client)
    book_id = await _make_book(client, token)
    group = (await _create_group(client, token, book_id)).json()
    notebook = (
        await _create_notebook(client, token, book_id, group_id=group["id"])
    ).json()

    response = await client.patch(
        f"{API}/books/{book_id}/latex-notebooks/{notebook['id']}",
        headers=_auth_headers(token),
...
@pytest.mark.asyncio
async def test_create_notebook_rejects_group_from_another_user(client):
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as other:
        ...
        response = await _create_notebook(
...
@pytest.mark.asyncio
async def test_delete_group_moves_notebooks_to_ungrouped(client):
    _, token = await _token(client)
    book_id = await _make_book(client, token)
    group = (await _create_group(client, token, book_id)).json()
    notebook = (
    ...
    deleted = await client.delete(
        f"{API}/books/{book_id}/latex-notebook-groups/{group['id']}",
        headers=_auth_headers(token),
    )
...
@pytest.mark.asyncio
async def test_notebooks_isolated_per_user(client):
    _, token1 = await _token(client)
    book_id = await _make_book(client, token1)
    group = (await _create_group(client, token1, book_id)).json()
    notebook = (
        await _create_notebook(client, token1, book_id, group_id=group["id"])
    ).json()
...
@pytest.mark.asyncio
async def test_notebooks_cascade_on_book_delete(client):
    _, token = await _token(client)
    book_id = await _make_book(client, token)
    group = (await _create_group(client, token, book_id)).json()
    assert (
    ...
    async with AsyncSessionLocal() as db:
        notebooks = (
            await db.scalars(
                select(LatexNotebook).where(LatexNotebook.book_id == book_id)
            )
        ).all()
        groups = (
            await db.scalars(
...
@pytest.mark.asyncio
async def test_reader_state_accepts_latex_panel(client):
    _, token = await _token(client)
    book_id = await _make_book(client, token)
    group = (await _create_group(client, token, book_id)).json()
    notebook = (
    ...
    assert state["panel"] == "latex"
    assert state["latex"]["active_id"] == notebook["id"]
    assert state["latex"]["active_group_id"] == group["id"]
    assert state["latex"]["search"] == "problem"
